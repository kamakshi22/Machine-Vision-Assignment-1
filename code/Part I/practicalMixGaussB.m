...
for (cData =1:nData)
    ...
    data(:,cData) = mixGauss.cov(h) *randn(1)+ mixGauss.mean(h);
end;
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        thisData = data(1,cData);
        %replace this:
        Z=0;
        for (cGauss = 1:k)
       
            likelihood(cGauss,cData)= mixGaussEst.weight(cGauss) * getGaussProb(thisData,mixGaussEst.mean(cGauss),mixGaussEst.cov(:,:,cGauss));
            Z= Z+ likelihood(cGauss,cData);
        end;
        
        responsibilities(:,cData) = likelihood(:,cData)/Z;
   end;
   ...
   for (cGauss = 1:k) 
        ...
        mixGaussEst.cov(1,1,cGauss) = sum (responsibilities(cGauss,:).*((data - mixGaussEst.mean(cGauss)).^2))/sum(responsibilities(cGauss,:));
   end;
...
for(cData = 1:nData)
    ...
    for (cGauss= 1:mixGaussEst.k)
        ...
        logLike = logLike+ (thisQ(cGauss) * log(likelihood(cGauss,cData)/thisQ(cGauss))); 
    end
   %add to total log like
    bound = bound+logLike;        
end;
